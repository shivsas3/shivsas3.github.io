# shivsas3.github.io
# Shiv Sastry — Engineering Portfolio

Live site: https://shivsas3.github.io

A running collection of independent mechanical engineering projects — reverse engineering, CAD, dimensioning, and design work done outside of coursework.

## About

I'm a sophomore at Lehigh University pursuing a dual degree in Mechanical Engineering and Finance through the Integrated Business and Engineering Honors Program. I also contribute to Lehigh's Formula SAE aerodynamics subsystem and write [The Parc Fermé Report](https://parcfermereport.substack.com), an independent publication analyzing Formula 1 technical regulations and strategy.

## Projects

- **Self-Locking Tape Measure — Reverse Engineering & Part Definition (2023)** — Full product teardown and dimensioning of an 8-component mechanism using a dial caliper, with tolerances derived at each mating interface and a complete 2D/3D part definition built in Onshape.

More projects will be added here over time.

## Contact

- Email: shiv.sas3@gmail.com
- LinkedIn: [linkedin.com/in/shiv-sastry](https://linkedin.com/in/shiv-sastry)
